...
async function regionalUpdateJob() {
  ...
  try {
    ...
    for (const region of regions) {
      ...
      const findFuel = (fuel) =>
        generationmix.find((item) => item.fuel === fuel)?.perc || 0;
      ...
      const cleanerTotal = parseFloat(
        (biomass + nuclear + hydro + solar + wind).toFixed(2)
      );
    ...
    console.log(
      `[${new Date().toISOString()}] Regional Update job completed successfully`
    );
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Error in Regional Update job:`,
      error.message
    );
  }
}
...
async function regionalDailyTotalsJob() {
  console.log(
    `[${new Date().toISOString()}] Starting Regional Daily Totals job...`
  );

  try {
    // Get distinct regions
    const regionsResult = await pool.query(
      "SELECT DISTINCT region FROM public.live ORDER BY region ASC"
    );
    ...
    console.log(
      `[${new Date().toISOString()}] Regional Daily Totals job completed successfully`
    );
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Error in Regional Daily Totals job:`,
      error.message
    );
  }
}
